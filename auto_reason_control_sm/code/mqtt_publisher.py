...
context = zmq.Context()
logger = logging.getLogger("main.mqtt_publisher")


class MQTTPublisher(multiprocessing.Process):
    def __init__(self, config, zmq_conf):
        super().__init__()

        mqtt_conf = config['mqtt_publisher']
        ...
        while run:
            while self.zmq_in.poll(50, zmq.POLLIN):
                try:
                    ...
                    topic = chevron.render(urljoin(self.topic_base, msg_topic), {**msg_payload, **self.constants})
                    logger.debug(f'Sending topic:{topic} msg:{msg_payload}')
